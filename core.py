...
#
# modified by Thierry
# modified by Dominique 03/03/20016
# major change pep8 correction
#
#*****************************************************************************
...
class Core:
    ...
    def __init__(self, domain, codomain, edge_map=None,
                 inv_edge_map=None, vertex_map=None,
        ...
        if edge_map is None:
            edge_map = \
                domain.difference_of_marking(codomain).tighten().edge_map()
        ...
        self._inv_graph_map = GraphMap(codomain, domain,
                                       inv_edge_map, inv_vertex_map)

        self._signed_ends = {}  # dictionary: keys = edge labels
        # and inverse labels of domain
        # signed_ends[x] = +- cylinder
        # decomoposition of image of edge_map
    ...
    def __str__(self):
        ...
        #for x in self._core_slice.keys(): # doesn't work right as some
        # letters in the alphabet are not used to label edges
    ...
    def _build_endmap(self, consolidate=False):
        ...
        for x in self._inv_graph_map.domain().edge_labels():
            ...
            for a in inv_image_x:
                ...
                if vp == '' or vp[-1] != X:  # coming from + side,
                    # no cancellation
                ...
                # repeat for inverse, we've added to the
                # vanishing path because we are
        ...
        if consolidate:
            for a in self._signed_ends.keys():
                removed_end = True
                N = 2 * inv_alph.cardinality()  # should take into account
                # the actual vertex...
                while removed_end:
                    removed_end = False
                    prefix = []
                    for e in self._signed_ends[a]:
                        prefix.append(e[:-1])
                    prefix_counts = dict((p, prefix.count(p))
                                         for p in set(prefix))
                    for p in prefix_counts.keys():
                        ...
                        if prefix_counts[p] == N - 2 and \
                                p_opp in self._signed_ends[a]:
                            removed_end = True
                            for e in self._signed_ends[a]:
                                if e[:-1] == p:
                                    removed_ends.append(e)
    ...
    def _build_core(self):
        ...
        for x in self._graph_map.domain().edge_labels():
            ...
            for e in ends[x]:
                common_len = \
                    self._inv_graph_map.domain().common_prefix_length(
                        e, common)
    ...
    def end_map(self, e=None):
        """
        Returns the image as a union (+) and difference (-)
        of cylinders of the
        one-sided cylinder determined by e if specified.
        Else returns the dictionary
        defined by the map on ends.
        """
        if e is None or e not in self._signed_ends.keys():
    ...
    def core_slice(self, e=None):
        ...
        of core slices.
        """
        if e is None or e not in self._core_slice.keys():
    ...
    def volume(self, e=None):
        ...
        """
        if e is None or e not in self._core_slice.keys():
            return sum(len(self._core_slice[x].edges())
                       for x in self._core_slice.keys())  # intersection number
        else:
    ...
    def rose_map(automorphism):
        """
        Returns the core of the rose and the rose acted
        upon by the corresponding
        automorphism.
        """
